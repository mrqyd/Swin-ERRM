...
class DAHead(nn.Module):
    ...
    def forward(self, x):
        ...
        output = nn.functional.interpolate(output, scale_factor=32, mode="bilinear", align_corners=True)
...
class DAnet(nn.Module):
    def __init__(self, num_classes,pretrained=True):
        ...
        #     resnet50(pretrained=pretrained, replace_stride_with_dilation=[False, True, True]),
        #     return_layers={'layer4': 'stage4'}
        # )
        self.ResNet18 = IntermediateLayerGetter(
            resnet18(pretrained=pretrained),
            return_layers={'layer4': 'stage4'}
        )
        # self.decoder = DAHead(in_channels=2048, num_classes=num_classes)
        self.decoder = DAHead(in_channels=512, num_classes=num_classes)

    def forward(self, x):
        feats = self.ResNet18(x)
...
if __name__ == "__main__":
    ...
    # model = ABCNet(n_classes=6,pretrained=False)
    # test = torch.randn(1, 3, 1024, 1024)
    # flops, params = profile(model, (test,))
    # print('flops: ', flops, 'params: ', params)
    # print('flops: %.2f M, params: %.2f M' % (flops / 1000000.0, params / 1000000.0))

    # 参数统计
    net = DAnet(num_classes=6,pretrained=False)
    # net.cpu()
    # y = net(torch.randn(1, 3, 1024, 1024))
    # print(type(y))
    # # print(y.size())
    summary(net, (1, 3, 128, 128))
