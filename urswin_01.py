...
class DCFAM(nn.Module):
    def __init__(self, encoder_channels=(96, 192, 384, 768), atrous_rates=(6, 12)):
        ...
        self.SRU = SRU(oup_channels=384,group_num=384,gate_treshold=0.5)
        # self.SRU2 = SRU(oup_channels=1, group_num=1, gate_treshold=0.5)
    ...
    def forward(self, x1, x2, x3, x4):
        ...
        # idea1
        # out4 = self.conv4(x4) + self.down34(self.pa(self.down232(x2)))
        out4 = self.conv4(x4) + self.down34(self.SRU(self.down232(x2)))
        # idea1
        # out3 = self.pa(x3) + self.down231(self.ca(self.down12(x1)))
        out3 = self.SRU(x3) + self.down231(self.ca(self.down12(x1)))
...
def urswin_base(pretrained=True, num_classes=4, weight_path='pretrain_weights/stseg_base.pth'):
    # pretrained weights are load from official repo of Swin Transformer
    model = URSwin(encoder_channels=(128, 256, 512, 1024),
...
def urswin_small(pretrained=True, num_classes=4, weight_path='pretrain_weights/stseg_small.pth'):
    model = URSwin(encoder_channels=(96, 192, 384, 768),
                   num_classes=num_classes,
                   embed_dim=96,
                   depths=(2, 2, 18, 2),
                   num_heads=(3, 6, 12, 24),
...
def urswin_tiny(pretrained=True, num_classes=4, weight_path='pretrain_weights/stseg_tiny.pth'):
    model = URSwin(encoder_channels=(96, 192, 384, 768),
                   num_classes=num_classes,
                   embed_dim=96,
                   depths=(2, 2, 6, 2),
...
if __name__ == '__main__':
    model = urswin_tiny(pretrained=False)
